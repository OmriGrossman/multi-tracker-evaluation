...
OUTPUT_FILE = 'results/results_bytetrack.txt'
...
def run_ultralytics_bytetrack():
    """Run ByteTrack using Ultralytics wrapper"""
    print("Running ByteTrack via Ultralytics...")
    ...
    with open(OUTPUT_FILE, 'w') as f:
        for track in all_tracks:
            ...
            x1, y1, w, h = track['bbox']  # These are x,y,w,h
            conf = track['confidence']
            cls = track['class_id']  # This is the mapped class ID

            # Write in MOT Challenge format: frame, id, x, y, width, height, confidence, class_id, visibility
            f.write(f"{frame_id},{track_id},{x1:.2f},{y1:.2f},{w:.2f},{h:.2f},{conf:.3f},{cls},1\n")

    print(f"ByteTrack complete. Results saved to {OUTPUT_FILE}")


if __name__ == "__main__":
    start_time = time.time()
    run_ultralytics_bytetrack()
    end_time = time.time()
